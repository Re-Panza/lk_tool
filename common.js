...
/* =========================================
   LOGICA CLASSIFICA ONLINE (Google Sheets)
   ========================================= */
// Genera ID univoco per il dispositivo (UUID)
...
async function fetchLeaderboard() {
    try {
        ...
        console.error("Err DB:", e);
        return fallbackLeaderboard();
    }
}

// Salva punteggio e ottieni classifica aggiornata
async function saveScoreToDB(name, score) {
    const uid = getDeviceID();
    const ua = navigator.userAgent;
    // URL per salvare (usa query params per semplicità con Google Script)
    const url = `${ASSETS_CONFIG.dbUrl}?action=save&name=${encodeURIComponent(name)}&score=${score}&uid=${uid}&ua=${encodeURIComponent(ua)}`;
    
    try {
        // Usiamo POST per inviare, Google Script lo gestisce tramite doPost(e) -> handleRequest(e)
...
function fallbackLeaderboard() {
    return [
        {name: "Re Panza", score: 50},
        {name: "Generale", score: 40},
        {name: "Soldato", score: 30},
        {name: "Recluta", score: 10},
        {name: "Offline", score: 0}
    ];
}
...
function createGameOverlay() {
    ...
    overlay.innerHTML = `
        ...
        <div id="leaderboard-panel" style="
            ...
            width: 90%; max-width: 350px; background: #1e293b; border: 2px solid #fbbf24;
            border-radius: 20px; padding: 20px; z-index: 50; text-align: center;
        ...
        ">
            ...
            <div id="new-record-input" style="display:none; margin-bottom:15px;">
                ...
                <input type="text" id="player-name" placeholder="Il tuo nome..." maxlength="12" style="
                    padding: 10px; border-radius: 8px; border: 1px solid #475569; 
                    background: #0f172a; color: #fff; width: 60%; text-align: center; font-weight: bold;
                ">
                <button onclick="submitScore()" style="
                    padding: 10px 15px; background: #34d399; border: none; border-radius: 8px;
                    font-weight: bold; cursor: pointer; color: #0f172a;
                ">SALVA</button>
            </div>

            <div id="score-list" style="
                max-height: 220px; overflow-y: auto; text-align: left; 
                background: #0f172a; padding: 10px; border-radius: 10px; margin-bottom: 15px;
                font-family: monospace; font-size: 13px;
            "></div>

            <button id="close-lb-btn" style="
                width: 100%; padding: 12px; background: #fbbf24; color: #000; font-weight: 800;
...
window.runWithSponsor = function(callback) {
    ...
    gameTimeInterval = setInterval(() => {
        seconds--; document.getElementById('game-timer').innerText = seconds;
...
function endGame() {
    ...
    fetchLeaderboard().then(board => {
        ...
        // Verifica se il punteggio entra in classifica (Top 10 o > 0 se lista vuota)
...
function renderScores(board) {
    ...
    list.innerHTML = board.map((entry, i) => `
        <div class="lb-row">
            <span>${i+1}. ${entry.name.substring(0,12)}</span>
...
function runIntro() {
    ...
    intro.innerHTML = `
        <div style="position:absolute;top:0;left:0;width:100%;height:100%;z-index:1;"><img src="${ASSETS_CONFIG.introImg}" style="width:100%;height:100%;object-fit:cover;opacity:0.9;"></div>
        <div style="position:absolute;bottom:40px;left:10%;width:80%;z-index:10;display:flex;flex-direction:column;align-items:center;">
            ...
            <div style="position:relative;width:100%;height:10px;background:rgba(255,255,255,0.2);border-radius:5px;border:1px solid rgba(255,255,255,0.3);"><div id="intro-bar" style="width:0%;height:100%;background:linear-gradient(90deg,#fbbf24,#f59e0b);border-radius:5px;box-shadow:0 0 20px rgba(251,191,36,0.8);transition:width ${ASSETS_CONFIG.introDuration}ms linear;"></div><div id="intro-chicken" style="position:absolute;top:-35px;left:0%;font-size:32px;transition:left ${ASSETS_CONFIG.introDuration}ms linear;transform:translateX(-50%) rotate(15deg);filter:drop-shadow(0 5px 5px rgba(0,0,0,0.8));animation:bounceChicken 0.5s infinite alternate;">🍗</div></div>
        </div><style>@keyframes bounceChicken{from{transform:translateX(-50%) rotate(15deg) translateY(0);}to{transform:translateX(-50%) rotate(25deg) translateY(-5px);}}</style>
    `;
